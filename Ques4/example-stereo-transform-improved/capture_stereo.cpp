...
void func_name(int num_args, ...)
{
	va_list args;
	int num_cameras_available = 1;

    va_start(args, num_args);

    if (num_args == 1)
    {
    	num_cameras_available = 1;
    	frame_l=cvQueryFrame(capture_l);
    }
    else
    {
    	num_cameras_available = 2;
    	frame_l=cvQueryFrame(capture_l);
    ...
    if(computeDisparity && (num_cameras_available == 2))
...
void func_name_2(CvCapture *capture)
{
...
int main( int argc, char** argv )
{
    if(argc == 1)
    {
        ...
        capture_l = cvCreateCameraCapture(0);
    ...
    if(argc == 2)
    {
        ...
        //cvSetCaptureProperty(capture, CV_CAP_PROP_FRAME_WIDTH, HRES_COLS);
        //cvSetCaptureProperty(capture, CV_CAP_PROP_FRAME_HEIGHT, VRES_ROWS);
        capture_l = cvCreateCameraCapture(dev);
        cvSetCaptureProperty(capture_l, CV_CAP_PROP_FRAME_WIDTH, HRES_COLS);
        cvSetCaptureProperty(capture_l, CV_CAP_PROP_FRAME_HEIGHT, VRES_ROWS);
    ...
    if(argc >= 3)
    {
        ...
        while(1)
        {
        	func_name(2,devl, devr);

        	func_name_2(capture_l);
        	func_name_2(capture_r);
        }
